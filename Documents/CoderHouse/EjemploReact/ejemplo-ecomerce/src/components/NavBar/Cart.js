...
function Cart(){
    ...
    return (
        <div>
            ...
            { cartList.map(producto=> (<div key={producto.id}><li><h3>{producto.title}</h3><h3>Cantidad: {producto.cantidad}</h3><h3>Precio de tu item: U$D{producto.price * producto.cantidad}</h3><button onClick={()=>deletedItem(producto.id)}>Eliminar Producto</button></li></div>)) }
